...
class PreCheckTicketAPIView(generics.RetrieveUpdateDestroyAPIView):
    queryset = Order.objects.filter(is_active=True)

    def get(self, request, *args, **kwargs):
        order = self.get_object()
        html = render_to_string('ticket_pre_check.html', {
            'order': order,
            'details': order.details.filter(is_active=True),
            'ticket_type': OrderTicketAPIView.TicketType.IGV,
            'ticket_types': OrderTicketAPIView.TicketType()
        })
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = 'attachment; filename=Precuenta_{}-{}.pdf'.format(
            order.serie,
            str(order.correlative)
        )
        result = BytesIO()
